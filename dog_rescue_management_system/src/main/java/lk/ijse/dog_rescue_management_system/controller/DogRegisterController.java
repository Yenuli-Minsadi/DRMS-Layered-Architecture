...
import lk.ijse.dog_rescue_management_system.bo.custom.impl.DogBOImpl;
...
//      DogBOImpl dogBO = new DogBOImpl();
    DogBO dogBO = (DogBO) BOFactory.getBoFactory().getBo(BOFactory.BOTypes.DOG);
    ...
    void btnDogSaveOnAction(ActionEvent event) {
        ...
        try {
            boolean isSaved = dogBO.saveDog(dogDto);
            if (isSaved) {
                resetPage();
    ...
    private void loadNextId() throws Exception {
        String nextId = dogBO.getNextDogId();
        lblDogId.setText(nextId);
    }

    private void loadTableData() throws SQLException, ClassNotFoundException {
        ArrayList<DogDto> dogRegisterDtoArrayList = dogBO.getAllDogs();
    ...
    public void btnDogUpdateOnAction(ActionEvent actionEvent) {
        ...
        try {
            boolean isUpdated = dogBO.updateDog(dogDto);
            if (isUpdated) {
                resetPage();
    ...
    public void btnDogDeleteOnAction(ActionEvent actionEvent) {
        ...
        if (response.isPresent() && response.get() == ButtonType.YES) {
            ...
            String dogId = lblDogId.getText();
            try {
                boolean isDeleted = dogBO.deleteDog(dogId);

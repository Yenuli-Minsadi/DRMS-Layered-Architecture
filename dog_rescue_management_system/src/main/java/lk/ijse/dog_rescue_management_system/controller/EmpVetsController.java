...
import lk.ijse.dog_rescue_management_system.bo.custom.impl.VetBOImpl;
...
//    VetDAO vetDAO = new VetDAOImpl();
//      VetBOImpl vetBO = new VetBOImpl();
    VetBO  vetBO = (VetBO) BOFactory.getBoFactory().getBo(BOFactory.BOTypes.VET);
    ...
    void btnVetSaveOnAction(ActionEvent event) {
        ...
        try {
            boolean isSaved = vetBO.saveVet(vetDto);
            if (isSaved) {
                resetPage();
    ...
    private void loadNextId() throws Exception {
        String nextId = vetBO.getNextVetId();
        lblVetId.setText(nextId);
    }

    private void loadTableData() throws SQLException, ClassNotFoundException {
        ArrayList<VetDto> vetDtoArrayList = vetBO.getAllVets();
    ...
    public void btnVetDeleteOnAction(ActionEvent actionEvent) {
        ...
        if (response.isPresent() && response.get() == ButtonType.YES) {
            ...
            String vetId = lblVetId.getText();
            try {
                boolean isDeleted = vetBO.deleteVet(vetId);

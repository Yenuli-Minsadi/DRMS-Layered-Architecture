...
import lk.ijse.dog_rescue_management_system.bo.custom.impl.RescuerBOImpl;
...
//    RescuerDAO rescuerDAO = new RescuerDAOImpl();
//      RescuerBOImpl rescuerBO = new RescuerBOImpl();
    RescuerBO rescuerBO = (RescuerBO) BOFactory.getBoFactory().getBo(BOFactory.BOTypes.RESCUER);
    ...
    void btnRescuerSaveOnAction(ActionEvent event) {
        ...
        try {
            boolean isSaved = rescuerBO.saveRescuer(rescuerDto);
            if (isSaved) {
                resetPage();
...
//            lblRequestId.setText("");
    ...
    private void loadNextId() throws Exception {
        String nextId = rescuerBO.getNextRescuerId();
        lblRescuerId.setText(nextId);
    }

    private void loadTableData() throws SQLException, ClassNotFoundException {
        ArrayList<RescuerDto> rescuerDtoArrayList = rescuerBO.getAllRescuers();
    ...
    public void btnRescuerUpdateOnAction(ActionEvent actionEvent) {
        ...
        try {
            boolean isUpdated = rescuerBO.updateRescuer(rescuerDto);
            if (isUpdated) {
                resetPage();
    ...
    public void btnRescuerDeleteOnAction(ActionEvent actionEvent) {
        ...
        if (response.isPresent() && response.get() == ButtonType.YES) {
            ...
            String rescuerId = lblRescuerId.getText();
            try {
                boolean isDeleted = rescuerBO.deleteRescuer(rescuerId);

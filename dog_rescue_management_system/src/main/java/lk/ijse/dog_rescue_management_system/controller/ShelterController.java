...
//      ShelterBOImpl shelterBO = new ShelterBOImpl();
    ShelterBO shelterBO = (ShelterBO) BOFactory.getBoFactory().getBo(BOFactory.BOTypes.SHELTER);
    ...
    void btnShelterSaveOnAction(ActionEvent event) {
        ...
        try {
            boolean isSaved = shelterBO.saveShelter(shelterDto);
            if (isSaved) {
                resetPage();
    ...
    private void loadNextId() throws Exception {
        String nextId = shelterBO.getNextShelterId();
        lblShelterId.setText(nextId);
    }

    private void loadTableData() throws SQLException, ClassNotFoundException {
        ArrayList<ShelterDto> shelterDtoArrayList = shelterBO.getAllShelters();
...
//            lblRequestId.setText("");
    ...
    public void btnShelterUpdateOnAction(ActionEvent actionEvent) {
        ...
        try {
            boolean isUpdated = shelterBO.updateShelter(shelterDto);
            if (isUpdated) {
                resetPage();
    ...
    public void btnShelterDeleteOnAction(ActionEvent actionEvent) {
        ...
        if (response.isPresent() && response.get() == ButtonType.YES) {
            ...
            String shelterId = lblShelterId.getText();
            try {
                boolean isDeleted = shelterBO.deleteShelter(shelterId);

...
import lk.ijse.dog_rescue_management_system.bo.custom.impl.MedicalRecordBOImpl;
...
//      MedicalRecordBOImpl medicalRecordBO = new MedicalRecordBOImpl();
    MedicalRecordBO medicalRecordBO = (MedicalRecordBO) BOFactory.getBoFactory().getBo(BOFactory.BOTypes.MEDICALRECORD);
    ...
    void btnMedRecSaveOnAction(ActionEvent event) {
        ...
        try {
            boolean isSaved = medicalRecordBO.saveMedRec(medicalRecordDto);
            if (isSaved) {
                resetPage();
    ...
    private void loadNextId() throws Exception {
        String nextId = medicalRecordBO.getNextMedRecId();
        lblMedRecId.setText(nextId);
    }

    private void loadTableData() throws SQLException, ClassNotFoundException {
        ArrayList<MedicalRecordDto> medicalRecordDtoArrayList = medicalRecordBO.getAllMedRecs();
    ...
    public void btnMedRecUpdateOnAction(ActionEvent actionEvent) {
        ...
        try {
            boolean isUpdated = medicalRecordBO.updateMedRec(medicalRecordDto);
            if (isUpdated) {
                resetPage();
                new Alert(Alert.AlertType.INFORMATION, "Record Saved Successfully!").show();
            } else {
    ...
    public void btnMedRecDeleteOnAction(ActionEvent actionEvent) {
        ...
        if (response.isPresent() && response.get() == ButtonType.YES) {
            ...
            String medicalRecordId = lblMedRecId.getText();
            try {
                boolean isDeleted = medicalRecordBO.deleteMedRec(medicalRecordId);
